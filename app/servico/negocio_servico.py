...
class NegocioServico:
    ...
    def deletar_negocio(
        ...
        Args:
            id (int): ID da ordem ou negócio
            all (bool): Flag que diz se são todas os negócios de uma ordem
            auth(bool): Flag que diz se o user está autenticado ou não.

        Raises:"""
        if auth:
            if all:
                self.negocio_repositorio.deletar_todos_negocios_ordem(id_ordem=id)
            else:
                self.negocio_repositorio.deletar_negocio(id_negocio=id)
        else:
            raise ExcecaoNaoAutenticado
